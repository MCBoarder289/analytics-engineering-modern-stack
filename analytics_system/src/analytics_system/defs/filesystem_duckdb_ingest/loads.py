...
repo_dir = pathlib.Path(__file__).parent.parent.parent.parent.parent.resolve()
warehouse_dir = repo_dir / ".data" / "warehouse"
warehouse_dir.mkdir(parents=True, exist_ok=True)

source_data_dir = pathlib.Path(__file__).parent.parent.parent.parent.parent.parent.resolve() / "data"

@dlt.source
def filesystem_calls_source(date_partition: Optional[str] | Optional[list[str]] = None):
    @dlt.resource
    def raw_calls():
        if date_partition:
            abs_path = os.path.abspath(os.path.join(source_data_dir, f"calls/day={date_partition}/")) # TODO: refactor this to be cleaner
